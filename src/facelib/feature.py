...
from facelib.util import asColumnMatrix
from facelib.operators import ChainOperator, CombineOperator
...
from facelib.lbp import LocalDescriptor, ExtendedLBP
...
class SpatialHistogram(AbstractFeature):
    def __init__(self, lbp_operator=ExtendedLBP(), sz = (8,8)):
        ...
        if not isinstance(lbp_operator, LocalDescriptor):
            raise TypeError("Only an operator of type facelib.lbp.LocalDescriptor is a valid lbp_operator.")
...
class Gaborfilter(AbstractFeature):
    ...
    def compute(self, X, y):
        ...
        for i in xrange(len(filters)):
            res_temp = self.process(X, filters)
            res.append(res_temp)
        ...
        self._num_components = pca.num_components
        # compute the new eigenspace
        self._eigenvectors = pca.eigenvectors
    ...
    def build_filters(self, ksize=9):
        filters = []

    	for theta in np.arange(0, np.pi, np.pi / 8):
            for lamda in np.arange(0, np.pi, np.pi/4):
                kern = cv2.getGaborKernel((ksize, ksize), 4.0, theta, 0, 0.5, 0, ktype=cv2.CV_32F)
                kern /= 1.5*kern.sum()
                filters.append(kern)

        return filters

    def process(self, image, filters):
        accum = np.zeros_like(image)
    	for kern in filters:
            fimg = cv2.filter2D(accum, cv2.CV_8UC3, kern)
    ...
    def __repr__(self):
        return "Gaborfilter (num_components=%s)" % (self.num_components)
