...
def grid_search(model, X, y, C_range=(-5,  15, 2), gamma_range=(3, -15, -2), k=5, num_cores=1):
    ...
    if not isinstance(model, PredictableModel):
        raise TypeError("GridSearch expects a PredictableModel. If you want to perform optimization on raw data use facelib.feature.Identity to pass unpreprocessed data!")
    if not isinstance(model.classifier, SVM):
        raise TypeError("GridSearch expects a SVM as classifier. Please use a facelib.classifier.SVM!")

    logger = logging.getLogger("facelib.svm.gridsearch")
    ...
    try:
        for p in g:
            ...
            if accuracy > best_accuracy:
                logger.info("best_accuracy=%s" % (accuracy))
                best_accuracy = accuracy
                best_parameter.C, best_parameter.gamma = C, gamma

            logger.info("%d-CV Result = %.2f." % (k, accuracy))
    except Exception:
        pass
    finally:
